...
namespace TRE
{
	...
	SwapChain::SwapChain(VkDevice LogicalDevice, GLFWwindow* Handle)
	{
		m_LogicalDevice = LogicalDevice;

		if (glfwCreateWindowSurface(RendererContext::GetVKInstance(), Handle, nullptr, &m_WindowSurface) != VK_SUCCESS)
		{
			assert(false);
		}
	}
	...
	uint32_t SwapChain::AccuireNextImage()
	{
		...
		if (auto Result = vkAcquireNextImageKHR(m_LogicalDevice, m_SwapChain, UINT64_MAX, m_Semaphores[m_CurrentBufferIndex].PresentComplete, VK_NULL_HANDLE, &Index); Result == VK_ERROR_OUT_OF_DATE_KHR)
		{
			Resize(m_Width, m_Height);
			return -1;
		}
