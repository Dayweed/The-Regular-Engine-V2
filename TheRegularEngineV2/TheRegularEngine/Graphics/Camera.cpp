...
namespace TRE
{
	...
	void CameraHelper::UpdateProjectionMatrix(BaseCamera& camera)
	{
		if (camera.m_IsPerspective)
		{
			...
			camera.m_CleanProj = glm::perspective(glm::radians(camera.m_Fov), camera.m_AspectRatio, camera.m_Near, camera.m_Far);
		}
		else
		{
...
#define far
	...
	Entity CameraSystem::GetMainCamera()
	{
	...
	void CameraSystem::MainCameraFollow(const glm::vec3& target, const float distance)
	{
		...
		if (mainCamera)
		{
			...
			cameraComponent.m_BaseCamera.m_FocalPoint = target;
			cameraComponent.m_BaseCamera.m_FocalLength = distance;
