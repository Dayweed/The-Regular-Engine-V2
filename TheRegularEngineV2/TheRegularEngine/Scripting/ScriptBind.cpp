...
#pragma region DirectPathfindingBinding
	...
	static float Engine_GetPathfindingCurrentDelay(CSEntityID id)
	{
		Entity entity = VALIDATEENTITY(id);
		if (!entity) return false;
		if (!entity->HasComponent<DirectPathfinding>())
		...
		return entity->GetComponent<DirectPathfinding>().m_CurrentTime;
	}
#pragma endregion
...
#pragma region Particle3D
	...
	static void Engine_GetVelocity3D(CSEntityID ID, glm::vec3* output)
	{
		const Entity& entity = VALIDATEENTITY(ID);
		if (!entity) return;
		...
		*output = entity->GetComponent<Particle3DComponent>().m_Velocity;
	}
...
#pragma endregion
	...
	void ScriptBind::RegisterFunctions()
	{
		...
		// Direct Pathfinding
		{
			...
			mono_add_internal_call("TRE.DirectPathfindingSystem::Engine_GetPathfindingCurrentDelay", Engine_GetPathfindingCurrentDelay);
		}
