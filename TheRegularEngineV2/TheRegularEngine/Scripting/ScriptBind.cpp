...
#pragma region Physics
    ...
    static void BindConstructBoxCollider(CSEntityID ID, glm::vec3 halfExtents, glm::vec3 offset)
    {
        const Entity& entity = VALIDATEENTITY(ID);
        if (!entity) return;
        ...
        if (entity->HasComponent<BoxCollider>())
        {
			PUBLISHERROR("There is BoxCollider in " + entity->GetName() + "!");
			return;
		}

		ECSSystemManager::Instance().GetSystem<PhysicsSystem>()->ConstructBoxCollider(entity, halfExtents, offset);
    }
